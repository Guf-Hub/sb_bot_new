...
from database import CheckCafe, Database
...
from fsm.cafe import CafeMorning, CafeEvening, CheckCafeReport
...
async def is_unique(message: Message, point: str, report_type: str, db: Database, date: str = None) -> bool:
    check = await db.check_cafe.is_unique_report_by_date(
        point=point,
...
async def morning_start(message: Message, state: FSMContext, db: Database):
    ...
    await state.set_state(CafeMorning.type_key)
    await message.reply('Выбери 👇', reply_markup=morning_type_menu)


@router.message(StateFilter(CafeMorning.type_key), F.text)
async def morning_type_key(message: Message, state: FSMContext):
    if message.text in ['Настройка', 'До 10', 'До 12']:
        ...
        await state.set_state(CafeMorning.point)
        await message.reply('Выбери точку 👇', reply_markup=await points_menu())
    else:
        await message.reply('Выбери 👇', reply_markup=morning_type_menu)


@router.message(StateFilter(CafeMorning.point), F.text)
async def morning_point(message: Message, state: FSMContext, db: Database):
    ...
    if await db.point.get_one(name=point):
        ...
        if type_key == 'Настройка':
            ...
            await state.set_state(CafeMorning.question_one)
            ...
            if await is_unique(message, point, report_type='Утренний до 10', db=db):
                ...
                async with ChatActionSender.upload_photo(chat_id=user_id, bot=bot):
                    ...
                    await state.set_state(CafeMorning.file1)
            ...
            if await is_unique(message, point, report_type='Утренний до 12', db=db):
                ...
                await state.set_state(CafeMorning.file3)
...
@router.message(StateFilter(CafeMorning.question_one), F.text)
...
@router.message(StateFilter(CafeMorning.file1), F.photo)
async def morning_file1(message: Message, state: FSMContext, db: Database):
    ...
    if type_key == 'Настройка':
        ...
        await state.set_state(CafeMorning.file2)
        await message.answer(check_q['mq_2'])


@router.message(StateFilter(CafeMorning.file2), F.photo)
async def morning_file2(message: Message, state: FSMContext, db: Database):
    ...
    async with ChatActionSender.typing(chat_id=user_id, bot=bot):
        ...
        report_id = await db.check_cafe.add(
            CheckCafe(
                add_date=add_date,
                type=report_type,
                point=point,
    ...
    if supervisor:
        ...
        kb = create_inline_kb(
            btns={'✅ Подтвердить?': f'checkCafeReport_{report_id}'},
        )
...
@router.message(StateFilter(CafeMorning.file3), F.video)
...
async def evening_start(message: Message, state: FSMContext, db: Database):
    ...
    await state.update_data(user_id=user.id)
    await state.set_state(CafeEvening.date)
...
@router.callback_query(F.data.startswith('eveningDate'), StateFilter(CafeEvening.date))
async def evening_date(query: CallbackQuery, state: FSMContext, db: Database):
    ...
    if user and user.status:
        ...
        await state.set_state(CafeEvening.point)
...
@router.message(StateFilter(CafeEvening.point), F.text)
    ...
    if message.video:
        ...
        async with ChatActionSender.typing(chat_id=user_id, bot=bot):
            ...
            if weekday != 'понедельник':
                ...
                await state.set_state(CafeEvening.file4)
                await message.answer(check_q['ev_4'])
    else:
        await message.answer('Загрузи одно видео!!!')


@router.message(StateFilter(CafeEvening.file4), F.video | F.photo | F.document | F.text)
async def evening_file4(message: Message, state: FSMContext):
    if message.video:
        await state.update_data(file4=message.video.file_id)
        await state.set_state(CafeEvening.file5)
        await message.answer(check_q['ev_5'])
    else:
        await message.answer('Загрузи одно видео!!!')


@router.message(StateFilter(CafeEvening.file5), F.video | F.photo | F.document | F.text)
async def evening_file5(message: Message, state: FSMContext):
    if message.video:
        await state.update_data(file5=message.video.file_id)
        await state.set_state(CafeEvening.file6)
        await message.answer(check_q['ev_6'])
    else:
        await message.answer('Загрузи одно видео!!!')


@router.message(StateFilter(CafeEvening.file6), F.video | F.photo | F.document | F.text)
async def evening_file6(message: Message, state: FSMContext):
    if message.video:
        await state.update_data(file6=message.video.file_id)
        await state.set_state(CafeEvening.file7)
        await message.answer(check_q['ev_7'])
    else:
        await message.answer('Загрузи одно видео!!!')


@router.message(StateFilter(CafeEvening.file7), F.video | F.photo | F.document | F.text)
...
@router.callback_query(F.data.startswith('checkCafeReport'), StateFilter(None))
async def check_report_start(callback_query: CallbackQuery, state: FSMContext, db: Database):
    ...
    if user.status:
        ...
        await state.set_state(CheckCafeReport.comment)
...
@router.message(StateFilter(CheckCafeReport.comment), F.text)
